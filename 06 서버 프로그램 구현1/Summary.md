...
#### 📘 정의
정보 은닉이란, **모듈 내부의 세부 구현 내용을 외부에 감추고, 필요한 정보나 기능만을 공개하는 소프트웨어 설계 기법**이다.  
주로 **인터페이스를 통해서만 상호작용**하도록 하여, **모듈 간 결합도를 낮추고 독립성을 확보**하는 것이 목적이다.

#### 🧩 정보 은닉의 목적

- **모듈 독립성 강화**: 내부 변경이 외부에 영향을 주지 않음
- **결합도(Coupling) 감소**: 모듈 간 상호 의존성 최소화
- **유지보수성 향상**: 버그 수정이나 기능 변경 시 영향 범위 축소
- **보안성 및 안정성 강화**: 외부에서 내부 구조 접근 차단

#### 🧩 정보 은닉의 대표 사례

| 사례 | 설명 |
|------|------|
| **클래스의 private 변수** | 외부에서 직접 접근하지 못하게 숨기고, getter/setter 제공 |
| **캡슐화(Encapsulation)** | 속성과 메서드를 하나로 묶고, 외부에 필요한 것만 공개 |

> ✅ 시험에서는 캡슐화와 정보 은닉의 관계를 함께 묻는 문제 출제

#### 📝 기출 포맷 예시

- 다음 중 **정보 은닉의 효과 또는 목적**이 아닌 것은?
- 정보 은닉과 관련된 **설명 중 옳지 않은 것**은?
- 정보 은닉을 **적용한 설계 예시**로 알맞은 것은?

#### 🧠 용어 설명

- **정보 은닉(Information Hiding)**: 모듈 내부 구현을 숨기고 필요한 정보만 외부에 제공하는 설계 원리
- **캡슐화(Encapsulation)**: 데이터와 기능을 하나로 묶어 외부 접근을 통제하는 객체지향 개념
- **결합도(Coupling)**: 모듈 간 의존성의 정도, 낮을수록 좋음
